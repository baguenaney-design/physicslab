// Parses a topic's curriculum markdown into the structure the Concept and Practice panels render.
//
// Shared by every simulation. The markdown files under docs/content/ are the single source of
// truth for curriculum content and are reviewed by Peter Syrenne — nothing here writes back to
// them. Ordering, the Exam Tips section, and the position of pending-figure notes are all derived
// at parse time so each file stays exactly as authored.
//
// Extracted from momentumContent.js when projectile became the second topic. The logic is
// unchanged; only the name generalised (parseMomentumContent → parseTopicContent). A topic's own
// content module is now just a raw import and one call — see momentum/momentumContent.js.
...
// Derived from the headings, so a source file needs no reordering: momentum.md is authored
// IB-MC / AP-MC / AP-FRQ / IB-Paper-2 and renders IB-MC / IB-Paper-2 / AP-MC / AP-FRQ.
...
// `fallbackTitle` is used only when the file has no `# ` heading, which would be a malformed
// source file rather than a normal case.
export function parseTopicContent(markdown, fallbackTitle = 'Untitled topic') {
  ...
  // Exam tips live inline in Concept Summary in the source; the panel shows them as their
  // own section, so they are hoisted here rather than by editing the reviewed markdown.
...
// Total questions across every ready group. Drives the Practice nav sublabel, so it is counted
// from the parsed content rather than written in — the number stays honest when a review adds
// or cuts a question. Zero while every group is still pending.
export function countQuestions(content) {
  return content.questionGroups.reduce((total, group) => total + group.questions.length, 0)
}
